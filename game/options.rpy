...
define config.name = _("Once Upon A Kiss")

## A short name for the game used for executables and directories in the built
## distribution. This must be ASCII-only, and must not contain spaces, colons,
## or semicolons.

define build.name = "OUAKiss"
...
define config.save_directory = "OUAKiss-0140220260"
...
# Pausing a little when encounter ,. etc. when dialouge running
define natural_pause = 0.15

init python:
    def natural_say(text: str):
        return ''.join([char + (f'{{w={natural_pause}}}' if char in ".,!?:-" else '') for char in text])

    config.say_menu_text_filter = natural_say
